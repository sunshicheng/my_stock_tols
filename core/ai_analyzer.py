...
def analyze_stocks(
    ...
    system = (
        ...
        "包括：① 核心看点（可适度结合卦象、政策导向与所属板块是否在今日驱动板块内）② 潜在风险 ③ 短期操作建议（1-3个交易日）。"
        "若某只股票所属板块在今日驱动板块中或近期有明确利好/利空新闻，请在分析中体现。卦象仅作参考，不喧宾夺主。每只股票分析控制在100字以内。"
    )
...
def analyze_funds(
    ...
    system = (
        ...
        "包括：① 基金特点 ② 近期表现与政策/宏观/板块关联 ③ 短期配置建议。"
        "卦象仅作参考，不喧宾夺主。每只基金控制在80字以内。"
    )
